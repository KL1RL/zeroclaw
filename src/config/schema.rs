...
pub struct Config {
    ...
    /// Home Assistant REST API tool configuration (`[home_assistant]`).
    #[serde(default)]
...
/// Home Assistant REST API integration (`[home_assistant]` section).
///
/// The access token is loaded from the active workspace `.env` file so it stays
/// out of `config.toml`. Supported variable names: `HOME_ASSISTANT_ACCESS_TOKEN`,
/// `HOME_ASSISTANT_TOKEN`, or `HOME_ASSISTANT_API_KEY`.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonSchema)]
pub struct HomeAssistantConfig {
    /// Enable the `home_assistant` tool.
    #[serde(default, alias = "enable")]
    pub enabled: bool,
    /// Base URL for the Home Assistant instance, e.g. `https://ha.example.com`
    /// or `http://homeassistant.local:8123`.
    #[serde(default)]
    pub url: String,
    /// Disable strict TLS certificate validation for self-signed/private PKI.
    /// Default: false.
    #[serde(default)]
...
impl HomeAssistantConfig {
    pub fn validate(&self) -> Result<()> {
        ...
        if trimmed_url.is_empty() {
            anyhow::bail!("home_assistant.url must not be empty when home_assistant is enabled");
        }

        let parsed = reqwest::Url::parse(trimmed_url)
            .context("home_assistant.url must be a valid absolute URL")?;
...
mod tests {
    ...
    #[test]
    async fn home_assistant_config_default_has_correct_values() {
        let cfg = HomeAssistantConfig::default();
        assert!(!cfg.enabled);
        assert!(cfg.url.is_empty());
        assert!(!cfg.disable_strict_ssl);
    }
...
"#;
    ...
    #[test]
    async fn resolve_runtime_config_dirs_ignores_unsupported_activeworkspaces_marker() {
        let _env_guard = env_override_lock().await;
        let default_config_dir = std::env::temp_dir().join(uuid::Uuid::new_v4().to_string());
        let default_workspace_dir = default_config_dir.join("workspace");
        let marker_config_dir = default_config_dir.join("profiles").join("alpha");
        let unsupported_state_path = default_config_dir.join("activeworkspaces.toml");
        ...
        fs::write(&unsupported_state_path, toml::to_string(&state).unwrap())
            .await
            .unwrap();
